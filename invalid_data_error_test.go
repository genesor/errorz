...
func TestInvalidDataError(t *testing.T) {
	t.Run("AsErrorz", func(t *testing.T) {
		err, isErrorz := AsErrorz(NewInvalidDataError("code", "key"))
	...
	t.Run("no code", func(t *testing.T) {
		err := NewInvalidDataError("", "some error content")
		require.EqualError(t, err, "some error content")

		res := errors.Is(err, &InvalidDataError{})
		require.True(t, res)

		err2 := fmt.Errorf("wrap2: %w", fmt.Errorf("wrap1: %w", err))
		res = errors.Is(err2, &InvalidDataError{})
		require.True(t, res)

		res = IsInvalidDataError(err2)
		require.True(t, res)

		err3, ok := AsInvalidDataError(err)
		require.True(t, ok)
		require.Equal(t, "some error content", err3.Key)

		err4, ok := AsInvalidDataError(err2)
		require.True(t, ok)
		require.Equal(t, "some error content", err4.Key)

		err5, ok := AsInvalidDataError(fmt.Errorf("not a InvalidDataError"))
		require.False(t, ok)
		require.Nil(t, err5)

		err6 := errors.New("base error")
		err7 := WrapWithInvalidDataError(err6, "", "some error content")
		require.EqualError(t, err7, "some error content: base error")

		require.True(t, errors.Is(err7, &InvalidDataError{}))
		require.True(t, errors.Is(err7, &InvalidDataError{}))
		require.True(t, IsInvalidDataError(err7))

		err8, ok := AsInvalidDataError(err7)
		require.True(t, ok)
		require.Equal(t, "some error content", err8.Key)

		err9 := new(InvalidDataError)
		require.True(t, errors.As(err7, err9))
		require.Equal(t, "some error content", err9.Key)
	})

	t.Run("with code", func(t *testing.T) {
		err, ok := AsInvalidDataError(NewInvalidDataError("awesome_code", "error"))
		require.True(t, ok)
		require.Equal(t, "awesome_code", err.Code)
	})

	t.Run("with formatting", func(t *testing.T) {
		err, ok := AsInvalidDataError(NewInvalidDataErrorf("", "error %d", 1))
		require.True(t, ok)
		require.Empty(t, err.Code)
		require.Equal(t, "error 1", err.Key)

		err2, ok := AsInvalidDataError(WrapWithInvalidDataErrorf(err, "", "error %d", 2))
		require.True(t, ok)
